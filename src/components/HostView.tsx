...
import { startNewHand, advanceBettingRound, determineWinners, distributePot } from '../game/gameController';
...
export function HostView({ gameCode }: HostViewProps) {
  ...
  async function handleNextRound() {
    ...
    const result = advanceBettingRound(game, game.deck_state);
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-4">
            {game.status === 'waiting' && players.length >= 2 && (
              <button
                ...
                className="px-6 py-3 bg-white text-black font-light rounded-lg hover:bg-gray-200 transition-colors"
              >
                Start Hand
              </button>
            )}
            {game.status === 'in_progress' && game.betting_round !== 'river' && (
              <button
        ...
        <div className="text-center mb-8">
          <div className="text-sm font-light opacity-60 mb-2">
            {game.status === 'waiting'
              ? `Waiting for players (${players.length}/8)`
              : `${game.betting_round.toUpperCase()}`}
          </div>
        </div>
        ...
        {currentPlayer && game.status === 'in_progress' && (
          <div className="text-center mb-8">
            ...
            {actions.slice(0, 5).map((action) => {
              ...
              return (
                <div key={action.id} className="text-sm font-light opacity-40">
                  {player?.name} {action.action_type}
